...
let test_store_load_two_sessions () =
  ...
  let event1 = Event.create ~source ~session:session1 ~event_data:SampleEvent.startup in
  let event2 = Event.create ~source ~session:session2 ~event_data:SampleEvent.shutdown in
  let () = ok (Storage.store storage ~event:event1 ~serialize:SampleEvent.to_yojson) in
  let () = ok (Storage.store storage ~event:event2 ~serialize:SampleEvent.to_yojson) in
  let session1_load_result =
    ok
      (Storage.load_session
         ...
         ~deserialize:SampleEvent.of_yojson)
  in
  Alcotest.(check (list event_t)) "loaded vs. stored" [ event1 ] session1_load_result;
  let session2_load_result =
    ok
      (Storage.load_session
         ...
         ~deserialize:SampleEvent.of_yojson)
  in
  Alcotest.(check (list event_t)) "loaded vs. stored" [ event2 ] session2_load_result
;;
